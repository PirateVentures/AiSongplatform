...
export function listenRequirementHint(listenedSeconds: number, durationSeconds = 0): string {
  if (meetsListenRequirement(listenedSeconds, durationSeconds)) {
    return "Heard enough — you can keep the whole song";
  }
  ...
  if (remaining <= 0) return "Almost there — keep listening to your song";
  if (heard < 0.5) return "Play your song to unlock checkout";
  return `Keep listening (~${remaining}s more) to unlock checkout`;
}
